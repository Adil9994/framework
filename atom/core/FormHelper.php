...
class FormHelper
{
    ...
    public static function textareaBlock($label, $name, $inputAttrs = [], $divAttrs = []) {
        $divString = self::stringifyAttrs($divAttrs);
        $inputString = self::stringifyAttrs($inputAttrs);
        $html = '<div' . $divString . '>';
        $html .= '<label for="' . $name . '">' . $label . '</label>';
        $html .= '<textarea id ="' . $name . '" name="' . $name . '"'. $inputString . ' />'.'</textarea>';
        $html .= '</div>';
        return $html;
    }
    ...
    public static function checkboxBlock($label, $name, $check = false, $inputAttrs = [], $divAttrs = [])
    {
        $divString = self::stringifyAttrs($divAttrs);
        $inputString = self::stringifyAttrs($inputAttrs);
        $checkString = ($check) ? ' checked="checked"' : '';
        $html = '<div' . $divString . '>';
